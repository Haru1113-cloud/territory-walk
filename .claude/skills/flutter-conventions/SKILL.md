...
│   ├── walk_record.dart         1回の散歩の記録（日時・距離・面積・ルート・写真）
│   ├── ranking_entry.dart       ランキング/チャレンジ画面用の1ユーザー分の集計結果
│   └── cheer.dart               応援スタンプ(誰が誰に送ったか)
...
│   └── loop_detector.dart       ブレ除去フィルタ＋輪の自動/自己交差クローズ判定
├── state/                       アプリの状態管理
...
│   ├── walk_history_repository.dart     散歩記録のローカル保存(写真base64込み)
...
│   └── cheer_repository.dart            応援スタンプをFirestoreへ送信/購読
│                                          (いずれも無料のSparkプランのみで運用)
├── badges/                      バッジ(実績)の定義・達成判定（UIに依存しない）
│   ├── badge.dart                バッジ1件のデータモデル
│   └── badge_definitions.dart    バッジ一覧・達成条件・判定ロジック
├── challenges/                  週替わりチーム目標の計算（UIに依存しない）
│   └── weekly_challenge.dart     週の境界・目標値・今週分の絞り込み
├── screens/                     画面
│   ├── home_shell.dart           下部ナビゲーションバーで5画面を切り替える入れ物
│   ├── walk_screen.dart          地図＋操作パネル（「地図」タブ）
│   ├── ranking_screen.dart       獲得面積ランキング＋応援（「ランキング」タブ）
│   ├── challenge_screen.dart     週替わりチーム目標の進捗（「チャレンジ」タブ）
│   ├── history_screen.dart       散歩記録の一覧（「記録」タブ・振り返り段階1）
│   ├── history_detail_screen.dart 1件の散歩のルート・写真（段階2、push遷移）
...
└── widgets/                     screens内で使う部品
    ...
    └── person_area_tile.dart     ランキング/チャレンジ共通の「順位+名前+面積」タイル
```

下部ナビゲーションバー（`home_shell.dart`）導入後は、`badge_screen.dart`/
`history_screen.dart`/`ranking_screen.dart`/`challenge_screen.dart`はタブ
として直接埋め込まれる（`Navigator.push`では遷移しない）。
`history_detail_screen.dart`のような「タブの中からさらに1段掘り下げる」
画面だけ、従来どおり`Navigator.push`で遷移する。下部ナビゲーションバー
自体は画面下端に張り付けず、左右・下に余白を持たせた丸型のバーとして
浮かせて表示する（`Scaffold.bottomNavigationBar`に`SafeArea`+`Material`
で角丸・影を付けたものを渡している）。
