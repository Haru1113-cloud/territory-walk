...
「情報を伝える地図アプリ」ではなく「柔らかい自然光の下を歩く舞台」に
見えることを目指し、生成り・クリーム系の明るく落ち着いたトーンを
採用している（2026-07にユーザー要望で、それ以前のダーク×グロートーン
から全面刷新。地図タイルも暗い配色から明るい配色に切り替えた）。配色
トークンの一覧・使い分け・
...
2026-07に「他ユーザーの領土を閲覧できる」機能をFirebase(Firestore+匿名認証、
無料のSparkプランのみで運用)で追加した。以下は **引き続き実装しない**。

- 対戦機能（他プレイヤーの領土を歩いて奪う等のゲームルール）
- ログイン・ユーザーアカウント（表示名・プロフィール等は持たない。
  匿名認証のuidを「誰の領土か」の識別にのみ使う）
- 他プレイヤーのリアルタイムな現在地表示（見えるのは確定済みの領土のみ）

### 他ユーザーの領土閲覧の実装概要

- `lib/auth/auth_service.dart`: 匿名認証(`FirebaseAuth`)のラッパー。
  ログイン画面は持たず、`ensureSignedIn()`が端末ごとのuidを返すのみ。
- `lib/storage/remote_territory_repository.dart`: Firestoreの`territories`
  コレクションへの書き込み(`publish`)と全件購読(`watchAll`)を担当。
  ローカル永続化(`territory_repository.dart`)とは独立しており、
  Firestoreへの公開に失敗してもローカルの散歩体験には影響しない
  (`TeraWalkController._publishToRemote`でtry/catchしている)。
- `Territory.ownerId`: 持ち主のuid。ローカルにのみ保存されていた過去データ
  にはこの情報がないためnullable。
- セキュリティルール(`firestore.rules`): 読み取りは誰でも可能、書き込みは
  匿名認証済みかつ自分のownerIdでのみ許可。一度確定した領土は
  更新・削除できない(現状のアプリの挙動と一致させている)。

### 将来これらを足すときにネックになりそうな点

- **`TeraWalkController`が単一ユーザー前提の構造のまま**: 他ユーザーの
  領土は`othersTerritories`という別リストで持つ形にとどめており、
  対戦（奪い合い）のような自分/他人の状態を統合的に扱うロジックは
  まだない。対戦機能を足す際はここが最も大きな設計変更ポイントになる。
